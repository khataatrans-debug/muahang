...
(function () {
  ...
  window.DB_KEYS = {
    ...
    // Module Purchasing / Procurement (purchasing.html) — nhánh riêng, không đụng master_data_ (Trucking cũ)
    purchasingMaster : "purchasing_master_" + window.APP_ID,
    purchasingRules  : "purchasing_rules_"  + window.APP_ID,
    purchasingDocs   : "purchasing_docs_"   + window.APP_ID, // Firestore collection
  };
  ...
  const DEFAULT_MODULES = {
    trucking: true, purchasing: true, accountant: true, performance: true,
    overview: true, master: true, maintenance: true,
    fuel: true, driver: true, users: true, depot: true,ware: true
  };
  ...
  window.addEventListener("load", function() {
    ...
    // vì các trang (index.html, depot.html...) dùng Firebase Modular SDK, khởi tạo
    // riêng/độc lập, không gán vào window.firebase. app-config.js tự kết nối lấy,
    // luôn trỏ đúng project purchasing-4d698 (nơi admin.html quản lý "companies").
    Promise.all([
      ...
      const cfgApp = appMod.initializeApp({
        ...
        authDomain: "purchasing-4d698.firebaseapp.com",
        databaseURL: "https://purchasing-4d698-default-rtdb.asia-southeast1.firebasedatabase.app",
        projectId: "purchasing-4d698",
        storageBucket: "purchasing-4d698.firebasestorage.app",
  ...
  function applyModuleVisibility() {
    ...
    // trong từng trang (index.html/ware.html/depot.html...) tự quản lý ẩn/hiện
    // (biết cả ẩn LẪN hiện lại đúng lúc). Nếu đụng vào đây nữa sẽ có 2 hệ
    // thống cùng sửa 1 thẻ <a> mà không biết về nhau → sinh lỗi ẩn/hiện chập
    // chờn tùy thứ tự chạy trước-sau giữa các lần tải trang.
    ...
    if (currentModule && window.APP_MODULES[currentModule] === false) {
      // Tìm trang đầu tiên mà công ty này THỰC SỰ được cấp quyền, tránh redirect cứng về
      // index.html (sẽ gây vòng lặp nếu công ty không có module trucking, ví dụ công ty chỉ dùng Depot)
      const firstAllowed = Object.keys(MODULE_NAV_MAP).find(m => window.APP_MODULES[m] !== false);
      navigate(firstAllowed ? MODULE_NAV_MAP[firstAllowed] : "login.html");
      return;
    }
